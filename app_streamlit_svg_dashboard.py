...
entry_event = st.sidebar.text_input("퍼널 진입 이벤트", value="view_certificate_detail")
final_event = st.sidebar.text_input("퍼널 최종 이벤트", value="certificate_share")

st.sidebar.markdown("**CSV 업로드** (헤더 예: `distinct_id,event,time,utm_source,platform,referrer,properties...`)")
file = st.sidebar.file_uploader("mixpanel_raw.csv 선택", type=["csv"])

if file is None:
    st.info("좌측에서 CSV를 업로드하세요. 샘플 데이터가 없으면 차트가 렌더링되지 않습니다.")
    st.stop()
...
# ---- Parse time (UTC->KST or naive->UTC->KST) ----
def parse_to_kst(s: pd.Series) -> pd.Series:
...
if conv_df.empty:
    st.warning("퍼널 계산 결과가 비어 있습니다. 이벤트 이름을 확인해주세요.")
    st.stop()

# ---- Weekly aggregation (KST, week starts Monday) ----
conv_df["week"] = conv_df["entry_time"].dt.tz_convert("Asia/Seoul").dt.to_period("W-MON").dt.start_time
...
# Template with custom tokens (triple single quotes inside outer triple double quotes is safe)
template = '''
<style>
...
html = (
    ...
    .replace("__W__", str(W))
    .replace("__H__", str(H))
    .replace("__ML__", str(ML))
    .replace("__XR__", str(W - MR))
    .replace("__XLINE_Y__", str(H - MB))
...
st.divider()
st.caption("ⓘ 툴팁은 단일 `<div role='tooltip'>`를 재사용하고, 포인트에 `tabindex='0'`와 키보드 이벤트를 연결합니다.")
